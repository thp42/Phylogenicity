...
output_fasta_file = '/home/pythagoras/Documents/PhD/Evolution/Proteins/ITPKA/PSSM_A2.fasta'
iupred_path = "/home/pythagoras/Programs/iupred3"
psipred_path = "/home/pythagoras/Programs/psipred"
pssm_matrix_path = '/home/pythagoras/Documents/PhD/Evolution/Proteins/ITPKA/pssm_PAM30_A2.csv'
...
# Function to process a single sequence record, modified to keep only the best hit
def process_sequence_record(args):
    seq_record, pssm_matrix, iupred_path, psipred_path, pssm_cutoff, iupred_cutoff, anchor_cutoff, ss_cutoff, flanking_aa_size, flank_size  = args
    ...
    best_hit = None  # Variable to store the best hit for the current sequence
    best_pssm_score = float('-inf')  # Initialize with negative infinity

    for i in range(len(fragments)):
        fragment = fragments[i]           
        pssm_score = pssm_score_sequence(fragment, pssm_matrix)

        # Check if the pssm_score is valid before comparing
        ...
        if pssm_score >= pssm_cutoff:
            ...
            left_flank = iupred_scores[start_flank:i]  # Flanking region before the fragment
            right_flank = iupred_scores[i + fragment_size:end_flank]  # Flanking region after the fragment
            # Combine both flanking regions
            flanking_region_scores = left_flank + right_flank
            # Calculate the mean IUPRED score only for the flanking regions
            if len(flanking_region_scores) > 0:
                mean_iupred = np.mean(flanking_region_scores)
            else:
                mean_iupred = 1  # Handle cases where no flanking region exists (optional, depending on your use case)

            mean_anchor = np.mean(anchor_scores[i:i+fragment_size])

            mean_coil = np.mean([p['Prob_C'] for p in psipred_probs[i:i+fragment_size]])

            mean_helix = np.mean([p['Prob_H'] for p in psipred_probs[i:i+fragment_size]])

            mean_strand = np.mean([p['Prob_E'] for p in psipred_probs[i:i+fragment_size]])
            ...
            if (mean_iupred >= iupred_cutoff and mean_anchor >= anchor_cutoff and
                ...
                # Use new flanking_aa_size for writing the output
                ...
                # If this fragment has a better PSSM score, store it as the best hit
                if pssm_score > best_pssm_score:
                    best_hit = (fasta_header, fragment_with_output_flanks)
                    best_pssm_score = pssm_score

    return best_hit  # Return the best hit for this sequence
...
def main():
    ...
    pssm_cutoff, iupred_cutoff, anchor_cutoff = 18.82, 0.43686, 0.41004 
    ss_cutoff = {'coil': 0.50391, 'helix': 0.0, 'strand': 1.0}
    flank_size = 60
    flanking_aa_size = 6  # Number of flanking amino acids to include when writing output
    ...
    # Write only the best hit from each sequence to the output file
    with open(output_fasta_file, "w") as output_conn:
        for result in all_results:
            if result:  # Only write if there was a best hit found
                header, fragment = result
                output_conn.write(f"{header}\n{fragment}\n")

    print(f"Best hits written to {output_fasta_file}")
