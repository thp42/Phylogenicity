...
# Function to create a mapping from motif FASTA files
def create_id_motif_mapping(fasta_file):
    id_motif_map = {}
    with open(fasta_file, 'r') as file:
        for line in file:
            if line.startswith('>'):
                unique_id = line[1:].strip()
            else:
                motif = line.strip()
                if unique_id not in id_motif_map:
                    id_motif_map[unique_id] = set()
                id_motif_map[unique_id].add(motif)
    return id_motif_map

# Create mappings for each isoform
itpka_mapping = create_id_isoform_mapping("/home/pythagoras/Documents/PhD/Evolution/FL/ITPKA/ITPKA/ITPKA_ownAnnotation.fasta", "ITPKA")
itpkb_mapping = create_id_isoform_mapping("/home/pythagoras/Documents/PhD/Evolution/FL/ITPKA/ITPKB/ITPKB_ownAnnotation.fasta", "ITPKB")
itpkc_mapping = create_id_isoform_mapping("/home/pythagoras/Documents/PhD/Evolution/FL/ITPKA/ITPKC/ITPKC_ownAnnotation.fasta", "ITPKC")
...
# Create mapping for motifs
motif_mapping = create_id_motif_mapping("/home/pythagoras/Documents/PhD/Evolution/FL/ITPKA/All/ITPK_Motif.fasta")
...
for leaf in tree.iter_leaves():
    ...
    if data:
        ...
        motif_data = motif_mapping.get(unique_id, None)
        motifs = ",".join(motif_data) if motif_data else "NoMotif"
        leaf.name = f"{unique_id} {','.join(isoforms)} {organism_name} {motifs}"
    else:
        ...
        motif_data = motif_mapping.get(unique_id, None)
        motifs = ",".join(motif_data) if motif_data else "NoMotif"
        leaf.name = f"{unique_id} {organism_name} {motifs}"
...
modified_tree_file = "/home/pythagoras/Documents/PhD/Evolution/FL/ITPKA/All/ITPKA_TaxAligned_tree_plot_with_isoforms_and_motifs.treefile"
tree.write(outfile=modified_tree_file)

# Print the file paths for confirmation
print(f"Modified tree with isoforms and motifs saved as treefile: {modified_tree_file}")
