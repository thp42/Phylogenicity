...
def modify_headers(seqs):
    new_seqs = []
    for seq in seqs:
        # Replace spaces and special characters with underscores in the entire description
        new_name = re.sub(r'[^A-Za-z0-9]', '_', seq.description)
        new_seq = SeqRecord(seq.seq, id=new_name, description="")
        new_seqs.append(new_seq)
    return new_seqs

# Load your amino acid sequences
fasta_file = "/home/pythagoras/Documents/PhD/Evolution/Proteins/ITPKA/None/ITPK_NoneIsoform.fasta"
...
output_file = os.path.join(os.path.dirname(fasta_file), "ITPK-NoneIsoform.fasta")
